...
- **상황 A — 빈 쿼리**(텍스트·칩 둘 다 없음) → 프론트 실행 버튼 비활성 + 서버 400.
- **상황 B — 직군 인식 실패**(자유입력은 있으나 LLM이 아는 직군을 못 뽑고 칩도 없어 병합 태그 0개)
  → 서버 400 + 안내 "직군을 인식하지 못했어요. 직군 칩을 선택하거나 직군명을 포함해 다시 입력하세요."
  (태그 없는 전체 수집 경로는 만들지 않는다.)
- **gemma 파싱 실패/JSON 깨짐** → `parseQuery`가 `{tagIds:[], keywords:[원문]}`로 폴백.
  이후 칩이 있으면 진행, 없으면 위 상황 B로 차단. (scorer 견고화 패턴 재사용)
- 결과적으로 `runPipeline`/`wanted`에 도달하는 `SearchSpec.tagIds`는 항상 1개 이상.
